...
class Ventes:
    ...
    def cloture_journee(date_jour=None):
        ...
        cursor.execute("""SELECT COUNT(*) as count, SUM(total_facture) as total, SUM(quantite_vendue) as quanti,
                    ...
                    FROM Ventes WHERE Date(date_vente) = %s""", (date_jour,))
        ...
        statistiques_ventes_jour = {
            "Nombre total de ventes effectuées aujourdhui": total_ventes_jour,
            "Montant total des ventes effectuées aujourdhui": total_vente_jour,
            "Quantité totale vendue aujourdhui": quantite_vendue_jour,
            "Total des achats effectués aujourdhui": total_achat_jour,
            "Total des profits réalisés aujourdhui": total_profit_jour
        }
