...
class Medicament:
    ...
    def create_table_new_medicament(conn):
        try:
            ...
            conn.commit()
                ...
                print(f"Erreur lors de la création de la table Medicament: {e}")
    ...
    def ajouter_medicament_code_barre(conn, code_barre):
        try:
            ...
            print(f"Erreur lors de l'ajout du médicament avec le code-barres {code_barre}: {e}")
            return None
    ...
    def ajouter_medicament_data_frame(conn, dataframe):
        try:
            ...
            print(f"Erreur lors de l'ajout des médicaments à partir du DataFrame: {e}")
            return None
    ...
    def supprimer_medicament(conn, id_medicament):
        try:
            ...
            print(f"Erreur lors de la suppression du médicament avec l'ID {id_medicament}: {e}")
    ...
    def supprimer_medicament_url(conn, url_medicament):
        try:
            ...
            print(f"Erreur lors de la suppression du médicament avec l'URL {url_medicament}: {e}")
    ...
    def modifier_medicament(
        ...
        try:
            ...
            print(f"Erreur lors de la modification du médicament avec l'ID {id_medicament}: {e}")
    ...
    def extraire_medicament(conn, id_medicament):
        try:
            ...
            print(f"Erreur lors de l'extraction du médicament avec l'ID {id_medicament}: {e}")
            return None
    ...
    def effectuer_vente_medicament(conn, id_medicament, quantite_vendu):
        try:
            ...
            print(f"Erreur lors de la vente du médicament avec l'ID {id_medicament}: {e}")
    ...
    def test_existance_url(conn, url):
        try:
            ...
            print(f"Erreur lors de la vérification de l'existence de l'URL {url}: {e}")
            return False
    ...
    def effectuer_stock_medicament(conn, id_medicament, quantite_vendu):
        try:
            ...
            print(f"Erreur lors de l'ajout de stock pour le médicament avec l'ID {id_medicament}: {e}")
    ...
    def get_medicament_by_code_barre(conn, code_barre):
        try:
            ...
            print(f"Erreur lors de l'extraction du médicament avec le code-barres {code_barre}: {e}")
            return None
    ...
    def extraire_medicament_code_barre_like(conn, pattern):
        try:
            ...
            print(f"Erreur lors de l'extraction des médicaments avec le code-barres similaire à {pattern}: {e}")
            return []
    ...
    def extraire_medicament_nom_like(conn, pattern):
        try:
            ...
            print(f"Erreur lors de l'extraction des médicaments avec le nom similaire à {pattern}: {e}")
            return []
    ...
    def extraire_medicament_nom_like_name(conn, pattern):
        try:
            ...
            print(f"Erreur lors de l'extraction des noms de médicaments similaires à {pattern}: {e}")
            return []
    ...
    def extraire_tous_medicament(conn, alphabet='A'):
        try:
            ...
            print(f"Erreur lors de l'extraction de tous les médicaments: {e}")
            return []
    ...
    def extraire_tous_new_medicament(conn):
        try:
            ...
            cursor.execute(
            "SELECT id_medicament, Code_EAN_13,  Nom, Présentation, PPV, Min_Stock FROM Medicament WHERE Code_EAN_13 IS NULL ORDER BY Nom;"
            )
            rows = cursor.fetchall()

            return [dict(row) for row in rows]
        except Exception as e:
            print(f"Erreur lors de l'extraction de tous les nouveaux médicaments: {e}")
            return []
    ...
    def extraire_medicament_quantite_minimale_sup_0(conn):
        try:
            conn = reconnexion_database(conn)
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            cursor.execute("SELECT id_medicament, Code_EAN_13, Nom, Présentation, PPV, Min_Stock, Stock_Actuel  FROM Medicament WHERE Min_Stock > 0;")
            rows = cursor.fetchall()

            return [dict(row) for row in rows]
        except Exception as e:
            print(f"Erreur lors de l'extraction des médicaments avec une quantité minimale supérieure à 0: {e}")
            return []
    ...
    def extraire_medicament_quantite_minimale_repture(conn):
        try:
            ...
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            cursor.execute(
                "SELECT id_medicament, Code_EAN_13, Nom, Présentation, PPV, Min_Stock, Stock_Actuel  FROM Medicament WHERE Min_Stock > 0 and Stock_Actuel<Min_Stock; "
            )
            rows = cursor.fetchall()

            return [dict(row) for row in rows]
        except Exception as e:
            print(f"Erreur lors de l'extraction des médicaments en rupture de stock: {e}")
            return []
    ...
    def cloture_journee(conn):
        try:
            ...
            return {
                "Total des médicaments":  0,
                "Total des médicaments avec stock inférieur au minimum":  0,
                "Total des médicaments avec stock positif": 0,
                "Calcul du stock total disponible de tous les medicaments": 0,
            }
