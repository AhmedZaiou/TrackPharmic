...
class AffichageFacture:
    ...
    def show_facture_interface(self):
        ...
        self.table_widget_facture.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
    ...
    def remplir_table_produits(self):
        ...
        for index, produit in enumerate(produits):
            self.table_widget_facture.setItem(index, 0, QTableWidgetItem(produit["nom"]))
            self.table_widget_facture.setItem(index, 1, QTableWidgetItem(str(produit["quantite"])))
            self.table_widget_facture.setItem(index, 2, QTableWidgetItem(f"{produit['prix_unitaire']}"))
            total_ligne = produit["quantite"] * produit["prix_unitaire"]
            self.table_widget_facture.setItem(index, 3, QTableWidgetItem(f"{total_ligne}"))
