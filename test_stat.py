...
class Caisse:
    ...
    def cloture_journee(self): 
        ...
        result = {
            'Credit_situation': {
                'credit_max_autorise': data['Client statistique']['credit_max_autorise'],
                'credit_actuel_pharmacie': data['Client statistique']['credit_actuel_pharmacie'],
                'total_restant_a_payer': data['Credit statistique']["Total restant à payer aujourd'hui"],
                'total_paiements_effectues': data['Paiment statistique']["Total des paiements effectués aujourd'hui en Dhs"],
                'nombre_paiements': data['Paiment statistique']["Nombre total de paiements effectués aujourd'hui"]
            },
            'Stock_situation': {
                'total_medicaments': data['Medicament statistique']['Total des médicaments'],
                'medicaments_stock_inferieur_min': data['Medicament statistique']['Total des médicaments avec stock inférieur au minimum'],
                'medicaments_stock_positif': data['Medicament statistique']['Total des médicaments avec stock positif'],
                'stock_total_disponible': data['Medicament statistique']['Calcul du stock total disponible de tous les medicaments'],
                'quantites_en_stock': data['Stock statistique']['Quantités totales en stock aujourdhui'],
                'quantites_minimales_non_respectees': data['Stock statistique']['Quantités minimales non respectées aujourdhui'],
                'commandes_en_attente': data['Commande statistique']['statistique general']['commandes_en_attente']
            },
            'Vente_situation': {
                'nombre_ventes_effectuees': data['Vente statistique']['Nombre total de ventes effectuées aujourdhui'],
                'montant_total_ventes': data['Vente statistique']['Montant total des ventes effectuées aujourdhui'],
                'total_retours': data['Retour statistique']['Total des retours effectués aujourdhui'],
                'total_revenus_ventes_retours': data['Vente statistique']['Total des profits réalisés aujourdhui']
            },
            'Performance_salarie': {
                'total_operations_par_salarie': [{
                    ...
                    'commandes_en_attente': item['statistique']['commandes_en_attente'],
                    'total_paiements': next((x['total_paiements_salarie'] for x in data['Paiment statistique']["Total des paiements effectués aujourd'hui par salarié"] if x['id_salarie'] == item['salarie']), 0),
                    'total_retours': next((x['total_retours_salarie'] for x in data['Retour statistique']['Total des retours effectués aujourdhui par salarié'] if x['id_salarie'] == item['salarie']), 0)
    ...
    def generate_html(self,statistics_dict):
        ...
        """.format(statistics_dict["Client statistique"]["nombre_de_clients"],
        ...
        """.format(statistics_dict["Commande statistique"]["statistique general"]["date"],
                statistics_dict["Commande statistique"]["statistique general"]["total_commandes"],
                statistics_dict["Commande statistique"]["statistique general"]["commandes_recues"],
                statistics_dict["Commande statistique"]["statistique general"]["commandes_en_attente"])
        ...
        for salarie in statistics_dict["Commande statistique"]["statistique par salarie"]:
        ...
        """.format(statistics_dict["Credit statistique"]["Total restant à payer aujourd'hui"])
        ...
        """.format(statistics_dict["Paiment statistique"]["Total des paiements effectués aujourd'hui en Dhs"],
                statistics_dict["Paiment statistique"]["Nombre total de paiements effectués aujourd'hui"])

        for payment in statistics_dict["Paiment statistique"]["Total des paiements effectués aujourd'hui par salarié"]:
            html_content += """
                <tr>
                    <td>{}</td>
                    <td>{}</td>
                </tr>
            """.format(payment["id_salarie"], payment["total_paiements_salarie"])
        ...
        """.format(statistics_dict["Retour statistique"]["Total des retours effectués aujourdhui"],
                statistics_dict["Retour statistique"]["Nombre total de retours effectués aujourdhui"])

        for retour in statistics_dict["Retour statistique"]["Total des retours effectués aujourdhui par salarié"]:
            html_content += """
                <tr>
                    <td>{}</td>
                    <td>{}</td>
                </tr>
            """.format(retour["id_salarie"], retour["total_retours_salarie"])
        ...
        """.format(statistics_dict["Stock statistique"]["Total des achats pour la journée"],
                statistics_dict["Stock statistique"]["Total des ventes pour la journée"],
                statistics_dict["Stock statistique"]["Quantités totales en stock aujourdhui"],
                statistics_dict["Stock statistique"]["Quantités minimales non respectées aujourdhui"],
                statistics_dict["Stock statistique"]["Nombre Médicaments proches de la date dexpiration aujourdhui"])
        ...
        """.format(statistics_dict["Vente statistique"]["Nombre total de ventes effectuées aujourdhui"],
                statistics_dict["Vente statistique"]["Montant total des ventes effectuées aujourdhui"],
                statistics_dict["Vente statistique"]["Quantité totale vendue aujourdhui"],
                statistics_dict["Vente statistique"]["Total des achats effectués aujourdhui"],
                statistics_dict["Vente statistique"]["Total des profits réalisés aujourdhui"])
    ...
    def generate_htmla(self,data):
        ...
        <body>
            <div class="container">
                ...
                <!-- Section Crédit -->
                <h2>Situation de Crédit</h2>
                <table>
                    <tr>
                        <th>Crédit Max Autorisé</th>
                        <td>{data['Credit_situation']['credit_max_autorise']} €</td>
                    </tr>
                    <tr>
                        <th>Crédit Actuel en Pharmacie</th>
                        <td>{data['Credit_situation']['credit_actuel_pharmacie']} €</td>
                    </tr>
                    <tr>
                        <th>Total Restant à Payer</th>
                        <td>{data['Credit_situation']['total_restant_a_payer']} €</td>
                    </tr>
                    <tr>
                        <th>Total des Paiements Effectués</th>
                        <td>{data['Credit_situation']['total_paiements_effectues']} €</td>
                    </tr>
                    <tr>
                        <th>Nombre de Paiements</th>
                        <td>{data['Credit_situation']['nombre_paiements']}</td>
                    </tr>
                </table>
                ...
                <table>
                    <tr>
                        <th>Total des Médicaments</th>
                        <td>{data['Stock_situation']['total_medicaments']}</td>
                    </tr>
                    <tr>
                        <th>Médicaments en Stock Inférieur au Minimum</th>
                    ...
                    <tr>
                        ...
                        <td>{data['Stock_situation']['stock_total_disponible']}</td>
                    </tr>
                    <tr>
                        <th>Quantités en Stock</th>
                        <td>{data['Stock_situation']['quantites_en_stock']}</td>
                    </tr>
                    <tr>
                        <th>Quantités Minimales Non Respectées</th>
                        <td>{data['Stock_situation']['quantites_minimales_non_respectees']}</td>
                    </tr>
                    <tr>
                        <th>Commandes en Attente</th>
                        <td>{data['Stock_situation']['commandes_en_attente']}</td>
                    </tr>
                </table>

                <!-- Section Vente -->
                <h2>Situation des Ventes</h2>
                <table>
                    <tr>
                        <th>Nombre de Ventes Effectuées</th>
                        <td>{data['Vente_situation']['nombre_ventes_effectuees']}</td>
                    </tr>
                    <tr>
                        <th>Montant Total des Ventes</th>
                        <td>{data['Vente_situation']['montant_total_ventes']} €</td>
                    </tr>
                    <tr>
                        <th>Total des Retours</th>
                        <td>{data['Vente_situation']['total_retours']} €</td>
                    </tr>
                    <tr>
                        <th>Total des Revenus (Ventes - Retours)</th>
                        <td>{data['Vente_situation']['total_revenus_ventes_retours']} €</td>
                    </tr>
                </table>
